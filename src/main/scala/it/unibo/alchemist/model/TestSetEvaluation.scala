...
class TestSetEvaluation[P <: Position[P]] extends OutputMonitor[Any, P] {
  ...
  override def finished(environment: Environment[Any, P], time: Time, step: Long): Unit = {
    ...
    val accuracies =
      nodes(environment)
      .map(node => {
        val weights = node.getConcentration(new SimpleMolecule(Sensors.model))
        val data = layer.getValue(environment.getPosition(node))
        (weights, data)
      })
  ...
  private def evaluate(weights: Any, data: Any): Double = {
